...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Run the frozen post-validation GEXY Batch-5 opening heterogeneity audit. "
            ...
            "It makes no market-data request and cannot change the Batch-5 verdict."
        )
    )
    ...
    parser.add_argument(
        ...
        help="frozen Batch-5 classified-volume Greek coverage floor; default: 0.90",
    )
    ...
    output = args.data_dir / "gexy_spxw_tradeflow_batch5_heterogeneity_audit.csv"
    args.data_dir.mkdir(parents=True, exist_ok=True)
    result.to_csv(output, index=False)

    print("GEXY BATCH-5 OPENING HETEROGENEITY AUDIT — FROZEN POST-VALIDATION DIAGNOSTIC")
    ...
    print("STATUS: post-validation diagnostic; official Batch-5 verdict is unchanged")
    ...
    print(f"\nOUTPUT CSV: {output}")
    print("NO PAID DATA REQUESTS: this audit reads only existing local Batch-5 feature CSVs.")
    print("NO OBSERVATION REMOVAL: leave-one-out values are influence diagnostics only; official Batch-5 endpoints remain unchanged.")
