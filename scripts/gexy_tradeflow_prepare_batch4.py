...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Prepare frozen GEXY Batch-4 opening-window local feature files for all supplied dates. "
    ...
    if missing:
        raise SystemExit("BATCH-4 PREP ABORTED: missing required local inputs: " + ", ".join(missing))

    args.data_dir.mkdir(parents=True, exist_ok=True)
    print("GEXY BATCH-4 LOCAL PREPARATION")
    ...
    for day in args.dates:
        log_path = args.data_dir / f"gexy_spxw_{day}_batch4_prepare.log"
        log_parts: list[str] = []
    ...
    print("\nBATCH-4 LOCAL PREPARATION COMPLETE")
    print(f"DATES PREPARED: {len(args.dates)}")
    print("NO PAID DATA REQUESTS: all stages read only existing local caches.")
    print("NO VALIDATION ENDPOINTS EVALUATED: run the frozen Batch-4 validator separately after preparation.")
