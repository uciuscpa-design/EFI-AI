...
def _summarize(per_day: pd.DataFrame) -> pd.DataFrame:
    ...
    for endpoint, column in (
        ...
        ("B_ordinary_candidate", "endpoint_b_ordinary_spearman"),
    ):
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Evaluate the frozen GEXY Batch-4 opening-window validation from local feature CSVs. "
            "This validator is 15m only: Endpoint A is the historical momentum+raw partial Spearman; "
            "Endpoint B is the pre-specified ordinary hedge/return Spearman candidate. No 5m or other "
            "horizon is evaluated, and no market-data request is made."
        )
    )
    ...
    parser.add_argument(
        ...
        help="minimum classified contract volume with usable Greeks; frozen Batch-4 value: 0.90",
    )
    parser.add_argument(
        "--data-dir",
        type=Path,
    ...
    by_day_output = args.data_dir / "gexy_spxw_tradeflow_opening_validation_batch_4_by_day.csv"
    summary_output = args.data_dir / "gexy_spxw_tradeflow_opening_validation_batch_4_summary.csv"
    args.data_dir.mkdir(parents=True, exist_ok=True)
    per_day.to_csv(by_day_output, index=False)
    summary.to_csv(summary_output, index=False)

    print("GEXY OPENING-WINDOW VALIDATION BATCH 4 — FROZEN 15M ENDPOINTS")
    ...
    print("ENDPOINT A: hedge_delta_units partial Spearman controlling backward 1m return + raw signed contracts; expected negative")
    print("ENDPOINT B: ordinary hedge_delta_units vs 15m forward return Spearman; expected negative")
    print("STATUS: fresh untouched Batch-4 validation; endpoints were frozen before acquisition")
    ...
    print("\nDAY-BY-DAY BATCH-4 RESULTS")
    print(per_day[display_columns].to_string(index=False))
    print("\nSEPARATE ENDPOINT SIGN-STABILITY SUMMARY")
    ...
    print("INTERPRETATION LIMIT: hedge_delta_units is a liquidity-provider hedge proxy; this is not dealer inventory, causality, or a production-edge claim.")
