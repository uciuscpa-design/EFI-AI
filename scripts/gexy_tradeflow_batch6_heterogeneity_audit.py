...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Run the frozen post-validation GEXY Batch-6 opening heterogeneity audit. "
            ...
            "It makes no market-data request and cannot change the Batch-6 verdict."
        )
    )
    ...
    parser.add_argument(
        ...
        help="frozen Batch-6 classified-volume Greek coverage floor; default: 0.90",
    )
    ...
    output = args.data_dir / "gexy_spxw_tradeflow_batch6_heterogeneity_audit.csv"
    args.data_dir.mkdir(parents=True, exist_ok=True)
    result.to_csv(output, index=False)

    print("GEXY BATCH-6 OPENING HETEROGENEITY AUDIT — FROZEN POST-VALIDATION DIAGNOSTIC")
    ...
    print("STATUS: post-validation diagnostic; official Batch-6 verdict is unchanged")
    ...
    print(f"\nOUTPUT CSV: {output}")
    print("NO PAID DATA REQUESTS: this audit reads only existing local Batch-6 feature CSVs.")
    print("NO OBSERVATION REMOVAL: leave-one-out values are influence diagnostics only; official Batch-6 endpoints remain unchanged.")
