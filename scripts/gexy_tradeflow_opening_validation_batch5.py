...
def _summarize(per_day: pd.DataFrame) -> pd.DataFrame:
    ...
    for endpoint, column in (
        ...
        ("B_ordinary_stability", "endpoint_b_ordinary_spearman"),
    ):
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Evaluate the frozen GEXY Batch-5 opening-window validation from local feature CSVs. "
            "This validator is 15m only: Endpoint A is the historical momentum+raw partial Spearman; "
            "Endpoint B is the frozen ordinary hedge/return stability endpoint. No 5m or other horizon "
            "is evaluated, and no market-data request is made."
        )
    )
    ...
    parser.add_argument(
        ...
        help="minimum classified contract volume with usable Greeks; frozen Batch-5 value: 0.90",
    )
    parser.add_argument(
        "--data-dir",
        type=Path,
    ...
    by_day_output = args.data_dir / "gexy_spxw_tradeflow_opening_validation_batch_5_by_day.csv"
    summary_output = args.data_dir / "gexy_spxw_tradeflow_opening_validation_batch_5_summary.csv"
    args.data_dir.mkdir(parents=True, exist_ok=True)
    per_day.to_csv(by_day_output, index=False)
    summary.to_csv(summary_output, index=False)

    print("GEXY OPENING-WINDOW VALIDATION BATCH 5 — FROZEN 15M ENDPOINTS")
    ...
    print("ENDPOINT B: ordinary hedge_delta_units vs 15m forward return Spearman; sign-stability/heterogeneity endpoint")
    print("STATUS: fresh untouched Batch-5 validation; endpoints were frozen before acquisition")
