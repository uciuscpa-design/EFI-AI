...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Prepare frozen GEXY Batch-6 opening-window local feature files for all supplied dates. "
    ...
    if missing:
        raise SystemExit("BATCH-6 PREP ABORTED: missing required local inputs: " + ", ".join(missing))

    args.data_dir.mkdir(parents=True, exist_ok=True)
    print("GEXY BATCH-6 LOCAL PREPARATION")
    ...
    for day in args.dates:
        log_path = args.data_dir / f"gexy_spxw_{day}_batch6_prepare.log"
        log_parts: list[str] = []
    ...
    print("NO VALIDATION ENDPOINTS EVALUATED: run the frozen Batch-6 validator separately after preparation.")
