...
from packages.gexy.rolling import DELTA_FEATURE_GROUPS, choose_shrinkage, inner_purged_split


NY = ZoneInfo("America/New_York")

# Keep these identical to the frozen V3 day-ahead evaluator. This script is a
# diagnostic ablation only: feature families change, model settings do not.
WINDOW_ROWS = 180
VALIDATION_ROWS = 30
MIN_FIT_ROWS = 40
RIDGE_ALPHA = 100.0
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Run strict session-ahead ablations of frozen GEXY V3 feature families. "
            "Each scored session uses only earlier-session labels for fitting and gating."
        )
    )
    ...
    print("STRICT MULTI-SESSION FEATURE-GROUP ABLATION")
    ...
    print(f"GROUPS: {','.join(DELTA_FEATURE_GROUPS)}")
    ...
    for group_name, group_features in DELTA_FEATURE_GROUPS.items():
        for horizon in args.horizons:
            prepared, features, target = prepare_baseline_frame(
                raw,
                horizon_minutes=horizon,
                feature_candidates=group_features,
            )
                ...
                day_rows.append(
                    {
                        ...
                        "features_requested": len(group_features),
            ...
            combined = pd.concat(group_predictions, ignore_index=True)
            actual = combined["actual"].to_numpy(dtype=float)
            prediction = combined["prediction"].to_numpy(dtype=float)
            zero = combined["zero"].to_numpy(dtype=float)
            model_metrics = evaluate_predictions(actual, prediction)
            zero_metrics = evaluate_predictions(actual, zero)
            relevant_days = [
                row
                for row in day_rows
                if row["feature_group"] == group_name and row["horizon_minutes"] == horizon
            ]
    ...
    if not aggregate_rows:
        raise SystemExit("no feature group produced strict day-ahead predictions")

    aggregate = pd.DataFrame(aggregate_rows).sort_values(["horizon_minutes", "feature_group"])
    per_day = pd.DataFrame(day_rows).sort_values(["horizon_minutes", "feature_group", "session_date"])

    summary_path = Path("gexy_spxw_multiday_ablation_summary.csv")
    day_path = Path("gexy_spxw_multiday_ablation_by_day.csv")
    aggregate.to_csv(summary_path, index=False)
    per_day.to_csv(day_path, index=False)

    print("\nSTRICT FEATURE-GROUP ABLATION SUMMARY")
    display = [
        "horizon_minutes",
        "feature_group",
        "scored_sessions",
    ...
    print(
        "NOTE: This is a diagnostic ablation, not a new tuning pass. Feature-group definitions and "
        "all model/gating settings are fixed before evaluation; every scored date uses only earlier "
        "sessions. Do not select or retune a group on these dates and then claim them as fresh holdout evidence."
    )
