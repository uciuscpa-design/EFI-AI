...
def test_batch5_prepare_cli_is_frozen_and_local_only() -> None:
    repo_root = Path(__file__).resolve().parents[1]
    result = subprocess.run(
        [sys.executable, "scripts/gexy_tradeflow_prepare_batch5.py", "--help"],
        cwd=repo_root,
        capture_output=True,
        text=True,
        check=False,
...
def test_batch5_prepare_date_parser_preserves_order() -> None:
    from scripts.gexy_tradeflow_prepare_batch5 import _parse_dates
